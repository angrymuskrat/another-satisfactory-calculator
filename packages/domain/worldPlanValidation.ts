...
export function parseCatalogPlan(value: unknown, catalog: Catalog): Plan {
  ...
  if (plan.targets.some(target => !items.has(target.itemId)) || plan.sources.some(source => !items.has(source.itemId)) ||
      plan.batch?.items.some(item => !items.has(item.itemId)) || plan.exports?.some(item => !items.has(item.itemId)) ||
      plan.lines?.some(line => !recipes.has(line.recipeId)) || plan.settings.enabledRecipeIds.some(id => !recipes.has(id)) ||
      plan.settings.enabledBuildingIds.some(id => !buildings.has(id)) || Object.keys(plan.settings.resourceWeights).some(id => !catalog.items.some(item => item.id === id && item.raw))) {
    throw new Error('В плане есть неизвестные предметы, рецепты, здания или веса сырья.');
  }
