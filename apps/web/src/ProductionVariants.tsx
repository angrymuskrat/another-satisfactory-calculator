        ...
        return <article className="panel variant-card" key={variant.id} aria-labelledby={`variant-${variant.id}`}>
          ...
          {valid ? <>
            <dl className="variant-metrics">
              {result.products.map(product => <div key={product.itemId}><dt>{item(product.itemId)?.name ?? product.itemId}</dt><dd>{metric(product.rate, 6)} {unit(item(product.itemId))}{variant.id === 'economy' && <small>{difference(product.rate, baseline?.products.find(p => p.itemId === product.itemId)?.rate ?? 0)} к максимальному варианту</small>}</dd></div>)}
