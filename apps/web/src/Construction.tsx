...
export function Construction({ catalog, model, building }: { catalog: Catalog; model: ConstructionModel; building: boolean }) {
  ...
  const groups = orderDependencies(model.production.map(g => ({ recipeId: g.recipeId!, inputs: g.averageInputs, outputs: g.averageOutputs })));
    ...
    return <details className="production-step" key={group.id} open={building || undefined}>
      ...
      <div className="step-details">
        <div className="full-width"><p>Физических машин: <CopyNumber value={group.count} label={`${group.name}: число машин`} /> · Частота каждой: <CopyNumber value={group.clock} label={`${group.name}: частота`} suffix="%" /></p>
          <p>Активная доля времени: <CopyNumber value={group.activeDuty * 100} label={`${group.name}: активная доля времени`} suffix="%" />. Это средняя доля работы группы, а не настройка частоты.</p>
  ...
  return <div className={building ? 'construction-panel' : undefined}>
    ...
    {building && <section className="panel"><h3>Материалы зданий</h3><p>Известна стоимость {model.materials.knownMachines} из {model.materials.totalMachines} физических машин. {model.materials.complete ? 'Все показанные здания учтены.' : 'Ведомость неполная: неизвестные стоимости не приняты за ноль.'}</p>{model.materials.items.map(i => <div className="simple-row" key={i.itemId}><span>{item(i.itemId)?.name ?? i.itemId}</span><CopyNumber value={i.amount} label={`Материалы: ${item(i.itemId)?.name ?? i.itemId}`} suffix={item(i.itemId)?.fluid ? 'м³' : 'шт'} /></div>)}{model.materials.unknown.map(b => <p key={b.buildingId}>Стоимость неизвестна: {b.name} × {b.count}.</p>)}<p className="hint">Материалы всех показанных зданий, включая отмеченные построенными. Ленты, трубы, фундаменты, энергомодули и геометрия размещения в ведомость не включены.</p></section>}
  </div>;
}
...
export function CatalogStatus({ catalog }: { catalog: Catalog }) {
  ...
  })}<p>Материалы зданий: стоимость задана у {catalog.buildings.filter(b => 'buildCost' in b && Array.isArray(b.buildCost)).length} из {catalog.buildings.length} типов зданий и у {catalog.miners.filter(b => 'buildCost' in b && Array.isArray(b.buildCost)).length} из {catalog.miners.length} типов добытчиков. Наличие стоимости само по себе не подтверждает её сверку.</p></details>;
}
