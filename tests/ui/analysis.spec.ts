...
test('F04: области русского поиска, сворачивание клавиатурой, единицы и сравнение цепочки без изменения плана', async ({ page }) => {
  const plan = createDefaultPlan(catalog); plan.settings.objective = 'smooth-power'; plan.mode = 'target'; plan.targets[0].rate = 10;
  ...
  await expect(card).toContainText('40 шт/мин');
...
test('F06: отсутствующая медь предлагается конечным внешним источником и подтверждается пересчётом', async ({ page }) => {
  ...
  const report = page.getByRole('region', { name: 'Результаты анализа', exact: true });
  await report.getByText('Исходный план · Допустимое приближение', { exact: true }).click();
  await expect(report.locator('details').first().getByRole('row').filter({ has: page.getByRole('rowheader', { name: 'Выпуск: Медный слиток', exact: true }) })).toContainText('0 шт/мин');
...
test('F06: одиночное насыщение двух ограничений не выдается за доказательство, совместное расширение проверено', async ({ page }) => {
  ...
  plan.settings.powerLimit = 4; plan.settings.buildingLimits = { smelter: 1 }; plan.settings.clock = 250;
  await openPlan(page, plan);
